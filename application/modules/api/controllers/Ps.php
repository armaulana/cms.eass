		...
		if ($this->input->server('REQUEST_METHOD') == "GET") {
			if ($auth_key != '') {
				if ($auth_key == 'elayanandroid') {
					...
					// $this->db->where('tanggal_pengajuan', date("2019-01-02"));
		...
		if ($this->input->server('REQUEST_METHOD') == "POST") {
			if ($auth_key != '') {
				if ($auth_key == 'elayanandroid') {
					...
					$GetPengajuan = $this->model->GetSelect("id, fullname, workflow", "pengajuanps", "idpengajuan = '$idpengajuan'");
					if ($GetPengajuan->num_rows() > 0) {
						if ($GetPengajuan->row()->workflow == "50") {
							...
							if ($this->db->update('pengajuanps', array('workflow' => 60), "idpengajuan = '$idpengajuan'")) {
		...
		if ($this->input->server('REQUEST_METHOD') == "POST") {
			if ($auth_key != '') {
				if ($auth_key == 'elayanandroid') {
					...
					$GetPengajuan = $this->model->GetSelect("id, workflow", "pengajuanps", "idpengajuan = '$idpengajuan'");
					if ($GetPengajuan->num_rows() > 0) {
						if ($GetPengajuan->row()->workflow == "60") {
							...
							if ($this->db->update('pengajuanps', array('workflow' => 70), "idpengajuan = '$idpengajuan'")) {
								if ($this->db->insert('verifikasi', $data_insert) ) {
									// $result['result']['msg'] = 'Successfully';
									// $result['result']['success'] = '1';
									$result['result'][] = array('msg' => 'Successfully',
		...
		if ($this->input->server('REQUEST_METHOD') == "GET") {
			if ($token != '') {
				if ($token == 'elayanandroid') {
					$tahun = date('Y');				
					$result['disetujui'] = $this->model->GetSelect('count(idpengajuan) as x','pengajuanps',"year(tanggal_pengajuan) = '$tahun' and workflow >= 70")->row()->x;
				    $result['proses'] = $this->model->GetSelect('count(idpengajuan) as x','pengajuanps',"year(tanggal_pengajuan) = '$tahun' and workflow < 70 and workflow not in(15,25,0)")->row()->x;
				    $result['ditolak'] = $this->model->GetSelect('count(idpengajuan) as x','pengajuanps',"year(tanggal_pengajuan) = '$tahun' and workflow in(15,25)")->row()->x;	
				    //array_push($result);
				}else{
					$result['result']['msg'] = 'Authentication In Valid';
					$result['result']['success'] = '0';
...
/* End of file Ps.php */
/* Location: ./application/modules/api/controllers/Ps.php */