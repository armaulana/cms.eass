...
class Mod_kecamatan extends CI_Controller {

	var $order = array('id_kecamatan' => 'desc'); 
    var $table = 'bucket_ref_kecamatan';
    var $idq = 'lib_id';
    var $column_order = array(); 
    var $column_search = array('a.nama_kecamatan');
	...
	public function index(){
		...
		$data['menu_active_sub'] 		= 'Kecamatan';
	...
	function ajax_add(){
		...
		if(empty($data['error'])){
			...
			$data = array(
					'id_kecamatan'	=> $this->input->post('id_kota_fk').''.$this->input->post('nomor'),
	...
	function rubah($id){
		...
		if($cek->total >= 1 ){
		        ...
		        if($dt->id_kota_fk == '' or $dt->id_kota_fk == null or $dt->id_kota_fk == 0){
		        	...
		        	$selected = $this->MyModel->get_selected_by_id_kecamatan($dt->id_kecamatan);
	...
	function ajax_update(){
		...
		if(empty($data['error'])){
			$data = array(
					...
					'id_kota_fk' => strtoupper($this->input->post('id_kota_fk')),
					'status' => $this->input->post('status'),
