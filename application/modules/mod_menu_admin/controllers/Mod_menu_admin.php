...
class Mod_menu_admin extends CI_Controller {

	var $order = array('' => 'desc'); 
    var $table = 'master_menu_administrator';
    var $idq = 'id';
	...
	public function index(){
		...
		$user_data['title'] 				= 'Menu Administrator';
		$user_data['menu_active_parent'] 	= 'Menus';
		$user_data['menu_active_sub'] 		= 'Administrator Menus';
	...
	function recursiveDelete($id) {
		$query 	= $this->db->select('count(*) as total')->where('parent', $id)->get('master_menu_administrator')->row();
		$query2 = $this->db->select('count(*) as total')->where('parent', $id)->get('master_menu_administrator')->result();
		if ($query->total > 0) {
			foreach($query2 as $current){
				$this->recursiveDelete($current->id);
			}
		}
		$this->db->delete('master_menu_administrator', array('id' => $id)); 
	}
	...
	function save_menu(){
		if($_POST['id'] != ''){
			$data = array(
				...
				'tipe' => $_POST['tipe'],
				'newtab' => $_POST['newtab'],
				'exclusivepage' => $_POST['exclusivepage'],
			);
			$this->db->where('id', $_POST['id']);
			$this->db->update('master_menu_administrator', $data);
			...
			$arr['icon']  = $_POST['icon'];
			$arr['tipe']  = $_POST['tipe'];
			$arr['newtab']  = $_POST['newtab'];
			$arr['exclusivepage']  = $_POST['exclusivepage'];
			$arr['id']    = $_POST['id'];
		}else{
			$data = array(
				'label' => $_POST['label'],
			...
			$this->db->insert('master_menu_administrator', $data);
			...
			$arr['menu'] = '<li class="dd-item dd3-item" data-id="'.$id.'" >
							...
							<div class="dd3-content" style="background: #9575cd4d;border-radius: 1px;border: 1px solid #605ca8;"><span id="label_show'.$id.'">'.$_POST['label'].'</span>
								<span class="span-right">/<span id="link_show'.$id.'">'.$_POST['link'].'</span> &nbsp;&nbsp; 
									<a class="edit-button" id="'.$id.'" label="'.$_POST['label'].'" link="'.$_POST['link'].'" icon="'.$_POST['icon'].'" ><i class="fa fa-pencil"></i></a>
